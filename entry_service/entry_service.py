...
LOG_DEPTH_UPDATES = env_int("LOG_DEPTH_UPDATES", 0)
LOG_BOOK_TOP_LEVELS = env_int("LOG_BOOK_TOP_LEVELS", 0)
...
def _wall_persistent(sym: str, now_ts: float, raw: bool, wall_price: float) -> bool:
    ...
    return age >= RT_WALL_PERSIST_SEC
...
async def maybe_enter(pool: asyncpg.Pool, sym: str, m: dict, now_ts: float, now_mono: float) -> None:
    ...
    if st is None:
        ...
        armed[sym] = ArmState(
            armed_at_mono=now_mono,
            armed_at_ts=now_ts,
            arm_price=m["price"],
            arm_vol_10s=m["vol_10s"],
            last_wait_log_mono=0.0,
        )
        logging.warning(
