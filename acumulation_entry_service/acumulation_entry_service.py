...
def detect_accum(rows_chron: List[Row]) -> Tuple[bool, Dict, str]:
    """
    rows_chron: chronological (oldest -> newest) covering BASE_N candles ideally.
    We compute:
      - range_pct on last WIN_N
      - vol_rel (avg vq win / avg vq base)
      - buy_ratio_win on last WIN_N: sum(tbq)/sum(vq)
      - support touches near min low in win window
      - buy_ratio on those touch candles
      - higher lows (swing lows) on last WIN_N (or ACC_SWING_LOOKBACK)
    """
    if len(rows_chron) < max(WIN_N, BASE_N // 4):  # tolerate partial start, but need enough
    ...
    # Conditions for "accumulation phase"
    if range_pct > ACC_RANGE_PCT_MAX:
        return False, {
            "range_pct": range_pct, "vol_rel": vol_rel, "buy_ratio_win": buy_ratio_win,
            "support_price": support_price, "support_touches": support_touches,
            "buy_ratio_on_touches": buy_ratio_on_touches,
            "swing_lows_found": swing_found, "higher_lows": higher_lows
        }, f"range_too_wide({range_pct:.4f})"
    ...
    if not (ACC_VOL_REL_MIN <= vol_rel <= ACC_VOL_REL_MAX):
        return False, {
            ...
            "buy_ratio_on_touches": buy_ratio_on_touches,
            "swing_lows_found": swing_found, "higher_lows": higher_lows
        }, f"vol_rel_out_of_band({vol_rel:.3f})"
    ...
    if buy_ratio_win < ACC_BUY_RATIO_MIN:
        return False, {
            ...
            "buy_ratio_on_touches": buy_ratio_on_touches,
            "swing_lows_found": swing_found, "higher_lows": higher_lows
        }, f"buy_ratio_low({buy_ratio_win:.3f})"
    ...
    if support_touches < ACC_SUPPORT_TOUCHES_MIN:
        return False, {
            ...
            "buy_ratio_on_touches": buy_ratio_on_touches,
            "swing_lows_found": swing_found, "higher_lows": higher_lows
        }, f"not_enough_support_touches({support_touches})"
    ...
    if buy_ratio_on_touches < ACC_BUY_RATIO_TOUCH_MIN:
        return False, {
            ...
            "buy_ratio_on_touches": buy_ratio_on_touches,
            "swing_lows_found": swing_found, "higher_lows": higher_lows
        }, f"support_not_defended(buy_touch={buy_ratio_on_touches:.3f})"
    ...
    if not higher_lows:
        return False, {
            ...
            "buy_ratio_on_touches": buy_ratio_on_touches,
            "swing_lows_found": swing_found, "higher_lows": higher_lows
        }, f"no_higher_lows(swings={swing_found})"
...
async def on_closed_kline(pool: asyncpg.Pool, symbol: str, k: dict) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        # upsert features at ts = close_time
        if feats:
            await conn.execute(
                UPSERT_FEATURE_SQL,
        ...
        if ok:
            ...
            logging.warning(
                "🟦 ACCUM_PHASE %s | tf=%s range=%.2f%% vol_rel=%.2fx buy=%.2f touches=%d touch_buy=%.2f support=%.6f",
                symbol, TF,
...
async def ws_loop(pool: asyncpg.Pool) -> None:
    url = ws_url(SYMBOLS, TF)

    while True:
        try:
            async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
                logging.info("Connected to Binance (klines) | tf=%s | symbols=%d", TF, len(SYMBOLS))
