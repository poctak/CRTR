#!/usr/bin/env python3
# accumulation_volume_replay_grid_v2.py
# ------------------------------------------------------------
# Historical replay / dry-run grid search
# Strategy: VOLUME accumulation (enhanced with orderflow / wick / trade structure)
#
# Iterations: 768
# ------------------------------------------------------------
...
def load_config() -> Config:
    return Config(
        ...
        acc_support_touches_max=env_int("ACC_SUPPORT_TOUCHES_MAX", 5),
        support_touch_tolerance_pct=env_float("SUPPORT_TOUCH_TOLERANCE_PCT", 0.0025),
        support_defense_close_min_pct=env_float("SUPPORT_DEFENSE_CLOSE_MIN_PCT", 0.0015),
        touch_buy_ratio_min=env_float("TOUCH_BUY_RATIO_MIN", 0.50),
        ...
        touch_lower_wick_min=env_float("TOUCH_LOWER_WICK_MIN", 0.0010),
        touch_upper_wick_max=env_float("TOUCH_UPPER_WICK_MAX", 0.0060),
        min_last_avg_trade_quote=env_float("MIN_LAST_AVG_TRADE_QUOTE", 40.0),
...
GRID_CONFIG: Dict[str, List[Any]] = {
    "acc_range_max_pct": [0.030, 0.035],                  # 2
    "support_touch_tolerance_pct": [0.0025, 0.0035],      # 2
    "touch_buy_ratio_min": [0.50, 0.55],                  # 2
    "min_bounce_from_support_pct": [0.003, 0.004],        # 2
    "resistance_tests_min": [1, 2],                       # 2
    "acc_support_touches_min": [2, 3],                    # 2
    "acc_support_touches_max": [4, 5],                    # 2
    "touch_delta_ratio_min": [0.00, 0.05],               # 2
    "touch_lower_wick_min": [0.0010, 0.0020],            # 2
    "min_last_avg_trade_quote": [40.0, 80.0],            # 2
}
...
def analyze_symbol(candles: List[Candle], cfg: Config) -> Tuple[Optional[Dict[str, Any]], str]:
    ...
    score += 1 if touch_lower_wick >= max(cfg.touch_lower_wick_min, 0.002) else 0
